...
func postInstallationSetup(deps *defaultsetup.Dependencies, mariadb_config *mariadb_config.MariaDBConfigureConfig) error {
	lg, _ := logger.Get()
	terminal.Clear()
	...
	if err := defaultsetup.RunStandardConfiguration(ctx, mariadb_config); err != nil {
