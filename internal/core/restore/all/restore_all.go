...
// "all databases" backup flow (contains dumps for multiple databases).
func RestoreAll(options restoreUtils.RestoreOptions) error {
	lg, err := logger.Get()
	if err != nil {
	...
	lg.Info("Starting all database restore",
	...
	lg.Info("Starting all databases restore", logger.String("file", options.File))
	...
	lg.Info("All databases restore completed", logger.String("file", options.File))
	DisplayRestoreSummary(options, startTime, lg, &configDB)

	// Display backup metadata if available (skip database comparison for all-databases restore)
...
func metadataPath(filePath string) string {
	...
	// Backup metadata files are saved with the ".meta.json" suffix (see utils/backup/all_databases.go)
...
func DisplayRestoreOverview(options restoreUtils.RestoreOptions, startTime time.Time, filePath string, lg *logger.Logger) {
	...
	lg.Info("All databases restore overview",
...
func DisplayRestoreSummary(options restoreUtils.RestoreOptions, startTime time.Time, lg *logger.Logger, config *database.Config) {
	...
	lg.Info("All databases restore summary",
		logger.String("target_host", options.Host),
		logger.Int("target_port", options.Port),
		logger.String("target_user", options.User),
		logger.String("backup_file", options.File),
	...
	// as it would require querying each database individually
}
